...
if __name__ == '__main__':
    ...
    # Load articles
    pubmed_abstracts = pd.read_csv(snakemake.input.csv, index_col=0)

    # run open-source queries
    result = open_source_query(model_id="mistralai/Mixtral-8x7B-Instruct-v0.1",
                                      pubmed_abstracts=pubmed_abstracts)

    # Merge results
    final_result = pd.concat((result, process_result(result_df=result)), axis=1)
    final_result.to_csv(snakemake.output.csv, index=True)
